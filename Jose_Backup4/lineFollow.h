...
byte sensorCnt = 0;
byte lineCnt = 0;
#define readLineSensor() ({stpercent = 100; sensorCnt = 0; lineCnt=0;})
//#define lineFollow()   ({lineReading = PINA; stpercent = steerTable[lineReading];})


void lineFollow(){
    byte placeholder = PINA;
    sensorCnt = sensorSum[placeholder];
    lineCnt = lineNum[placeholder];
    if (placeholder) 
        lineReading = placeholder;
    else if (lineReading != B00000001 || lineReading != B10000000) 
        lineReading = placeholder;
    else 
        return;
