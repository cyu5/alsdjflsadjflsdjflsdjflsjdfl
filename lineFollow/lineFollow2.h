...
#define startTimer() ({\
        ...
        OCR3C = TCNT3 + (E_CLOCK*10/PRESCALER);         /* C is for line sensor */\
        TIMSK3 |= 0x06;     /* enable OC interrupt A and B */\
        TIMSK3 |= 0x08;     /* enable OC interrupt C */\
        })
#define setupTimer() ({\
        DDRL = 0xFF;     /*configure motors' port as output*/\
        ...
        TCNT3  = 0;       /* reset count */ \
        TCCR3B = 0x03;    /* normal mode, 64 prescaler (0x02 for prescalar = 8) */ \
...
volatile byte lineCnt = 0;     
volatile int steerTable[NUM_SENSORS] = {-739, -417, -236, -133, 133, 236, 417, 739}; 
volatile bool follow, followInt = false;
...
ISR(TIMER3_COMPC_vect)        //line sensor vector, takes too long
{
        if (!followInt) return;
        //calculate position
        lineCnt = 0;  
        ...
        {
            position = avg/sum;
            if (follow) stpercent = steerTable[position];
            else stpercent = 100;
        }

        OCR3C += min((long)periodCnt*10, (long)E_CLOCK * TIMEOUT / PRESCALER);
...
void accelMovi(byte dir, float dist, boolean overwrite = true, unsigned int n = 20, unsigned int startPeriod = 1000, unsigned char startSteps = 64){
    ...
    stackCnt = 0;   //unnecessary since TiMSK3 is 0
...
#define lineFollow() readLineSensor(); stpercent = steerTable[position]
